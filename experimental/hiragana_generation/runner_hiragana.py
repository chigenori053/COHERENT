"""
Hiragana Generation Experiment Runner

Executes the Hiragana Generation Experiment (46 Seion).
"""
...
# Import Hiragana Specifics
from experimental.hiragana_generation.attribute_mapping import HIRAGANA_MAPPING, get_attributes_for_hiragana, get_all_chars

def run_hiragana_experiment(output_dir="experimental/hiragana_generation/logs"):
    ...
    log_file = os.path.join(output_dir, f"hiragana_experiment_log_{timestamp}.csv")

    print(f"Starting Hiragana Generation Experiment [Mode: {config.mode}]")
    ...
    memory = DynamicHolographicMemory(encoder, attribute_lookup_fn=get_attributes_for_hiragana)
    diffusion = RefinementDiffusionDDIM(config)
    diagnostics = ExperimentDiagnostics(config)

    # 2. Populate Memory
    all_attributes = set()
    for attrs in HIRAGANA_MAPPING.values():
    ...
    with open(log_file, 'w', newline='') as f:
        ...
        for target_char in all_chars:
            run_id = f"{timestamp}_{target_char}" # filename safe char?
            # Hiragana chars are fine in filenames usually, but maybe unsafe on some FS.
            # Use index? or ascii map? "a", "ka"...
            # Let's use Hex code for safety if needed, but modern OS handles unicode.
            ...
            safe_name = f"{ord(target_char):x}" # Unicode hex
            np.savez_compressed(
    ...
    print("\n--- Hiragana Experiment Summary ---")
    print(results_summary)
    print(f"Log saved to: {log_file}")

if __name__ == "__main__":
    run_hiragana_experiment()
